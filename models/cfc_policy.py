...
# MlpPolicy is a class used in original LOKI experiment
    ...
    def _init(self, ob_space, ac_space, hid_size, num_hid_layers, gaussian_fixed_var=True):
        ...
        with tf.variable_scope('pol'):
            ...
            if gaussian_fixed_var and isinstance(ac_space, gym.spaces.Box):
                mean = tf.layers.dense(last_out, pdtype.param_shape()[
                ...
                pdparam = tf.layers.dense(last_out, pdtype.param_shape(
...
# CfCPolicy is a class used in our experiment based on the MlpPolicy for value function and CfC for policy

class CfCPolicy(MlpPolicy):
    def _init(self, ob_space, ac_space, hid_size, num_hid_layers, num_neurons=124, num_outputs=3, gaussian_fixed_var=True):
        super()._init(ob_space, ac_space, hid_size, num_hid_layers, gaussian_fixed_var)
        ...
        self.pdtype = pdtype = make_pdtype(ac_space)
        self.model.compile(optimizer=tf.keras.optimizers.Adam(0.01), loss='mean_squared_error')
        self.pdparam = self.model(self.ob)
        self.pd = pdtype.pdfromflat(self.pdparam)
        self._act = U.function([stochastic, self.ob], [self.pd.sample(), self.vpred])